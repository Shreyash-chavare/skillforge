...
// assume you have some AI utility for title generation
...
export const getOrGenerateTitles = async (req, res) => {
  try {
    const { topic } = req.params; // frontend sends topicId when clicking card
    const userId = req.user._id;    // from auth middleware (JWT/session)
    const forceRegenerate = req.query.regenerate === 'true'; // Allow forcing regeneration
    ...
    // 2. If already marked generated but titles are empty → regenerate
    if (Topicdoc.generated && (!Array.isArray(Topicdoc.titles) || Topicdoc.titles.length === 0)) {
      Topicdoc.generated = false;
      await Topicdoc.save();
    }
    
    // 3. If titles exist and not forcing regeneration → return existing titles
    if (!forceRegenerate && Array.isArray(Topicdoc.titles) && Topicdoc.titles.length > 0) {
      console.log(`Returning existing ${Topicdoc.titles.length} titles for topic: ${topic}`);
      return res.json({
    ...
    // 4. Force regeneration: clear existing titles
    if (forceRegenerate && Array.isArray(Topicdoc.titles) && Topicdoc.titles.length > 0) {
      console.log(`Force regenerating titles for topic: ${topic} (had ${Topicdoc.titles.length} old titles)`);
      Topicdoc.titles = [];
      Topicdoc.generated = false;
      await Topicdoc.save();
    }

    // 5. Generate titles using AI
    // Use topic's own difficulty (per-topic, not global)
    const topicDifficulty = Topicdoc.difficulty || "beginner";
    console.log(`Generating new titles for topic: ${Topicdoc.name}, difficulty: ${topicDifficulty}`);
    const generatedTitles = await generateTitles(Topicdoc.name, topicDifficulty);
    
    console.log(`Received ${generatedTitles.length} titles from AI`);

    const structuredTitles = generatedTitles.map((t, idx) => ({
      title: t.title,
      order: idx + 1,
      difficulty: t.difficulty, // store here
      completed: false,
      content: [],
      quiz: { questions: [] }
    }));
    ...
    console.log(`Saved ${structuredTitles.length} titles to database for topic: ${topic}`);

    // 7. Return first title
    return res.json({
...
export const getOrGenerateTitleContent = async (req, res) => {
  try {
    ...
    // If no content, generate minimal content fallback
...
export const getUserTopics = async (req, res) => {
  try {
    ...
    // Get all topics for the user with their titles and difficulty
...
// Update topic difficulty (per-topic)
export const updateTopicDifficulty = async (req, res) => {
  try {
    const userId = req.user._id;
    const { topic } = req.params; // topic name
    ...
    // Delete old content and mark for regeneration
    // Clear all titles, content, and quizzes to regenerate with new difficulty
    topicDoc.titles = [];
    topicDoc.generated = false;
    
    await topicDoc.save();

    // Also delete associated progress since content is being regenerated
