...
function ProfilePage() {
  ...
  useEffect(() => {
    const fetchuser = async () => {
      try {
        ...
        // Initialize difficulties for existing topics
        const existingDifficulties = {};
        if (user.data.preferences.fields) {
          // We'll need to fetch topic difficulties from topics endpoint
          // For now, initialize with beginner as default
        ...
        // Fetch daily progress to determine current streak
        try {
          ...
          // Get current streak from the most recent daily progress entry
  ...
  const handleFieldsChange = (e) => {
    ...
    // Only initialize difficulties for NEW topics (not existing ones)
    const newDifficulties = { ...topicsWithDifficulty };
    fieldsArray.forEach(field => {
      // Only add difficulty if it's a new topic (not in existing topics)
      if (!existingTopicNames.includes(field.toLowerCase()) && !newDifficulties[field]) {
        newDifficulties[field] = "beginner";
      }
    });
    
    // Remove difficulties for topics that are no longer in the list or are existing topics
  ...
  const handleUpdate = async (action) => {
    if (action === "edit") {
      setIsEditing(true);
      // Fetch current topics to identify which are existing vs new
      try {
        const token = localStorage.getItem("token");
        ...
        // Store existing topic names (case-insensitive comparison)
        ...
        // Only initialize difficulties for NEW topics (not in existing topics)
        const fieldsArray = update.fields.split(",").map((f) => f.trim()).filter((f) => f.length > 0);
        const newDifficulties = {};
        
        fieldsArray.forEach(field => {
          // Only add difficulty for topics that don't exist yet
    ...
    if (action === "save") {
      try {
        ...
        // Only send difficulties for NEW topics (not existing ones)
        // Filter out existing topics from topicDifficulties
        ...
        const updated = await api.put(
          ...
          {
            fields: fieldsArray,
            topicDifficulties: newTopicDifficulties, // Only new topics
          },
          { headers: { Authorization: `Bearer ${token}` } }
        );
    ...
    if (action === "cancel") {
      ...
      setTopicsWithDifficulty({}); // Clear difficulties
      setExistingTopicNames([]); // Clear existing topics
      setIsEditing(false);
    }
  };
      ...
      <div className="max-w-7xl mx-auto px-8 py-8">
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1">
            {/* Left Column - Profile Card (with integrated Quick Stats) */}
            <motion.div
                ...
                <div className="mb-6">
                  ...
                  {isEditing ? (
                    <>
                      ...
                      />
                      {/* Topic Difficulty Selectors - Only for NEW topics */}
                      {update.fields && (() => {
                        const fieldsArray = update.fields.split(",").map((f) => f.trim()).filter((f) => f.length > 0);
