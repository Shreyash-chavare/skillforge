...
const Score = () => {
  ...
  // Data passed from Quiz.jsx or backend response
  const { score, results } = location.state || {};
  
  console.log("Score page received:", { score, results }); // Debug log
        ...
        >
          ...
          <ul className="mt-2 space-y-2">
            {q.options.map((opt, i) => {
              ...
              // Correct answer
              if (i === correctIdx) {
                bg = "bg-green-200 border-green-600"; // correct option
                textColor = "text-green-800";
              } 
              // User's wrong selection
              else if (i === userIdx && !q.isCorrect) {
                bg = "bg-red-200 border-red-600"; // wrong chosen option
                textColor = "text-red-800";
              }
              // Check if user didn't answer this question
              else if ((userIdx === null || userIdx === undefined) && i === correctIdx) {
                bg = "bg-yellow-200 border-yellow-600"; // correct answer but user didn't select it
