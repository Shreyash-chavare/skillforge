...
export default function Preferences() {
  ...
  // Update topics with difficulty when fields input changes
  const handleFieldsChange = (e) => {
    const value = e.target.value;
    setFieldsInput(value);
    
    // Parse fields and initialize difficulties
    const fieldsArray = value
      .split(",")
      .map((f) => f.trim())
      .filter((f) => f.length > 0);
    
    // Initialize difficulties for new topics (default: beginner)
    ...
    // Remove difficulties for topics that are no longer in the list
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Use navigate instead of window.location for better React behavior
