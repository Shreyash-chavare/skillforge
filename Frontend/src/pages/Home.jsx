...
const Home = () => {
  ...
  const handleDifficultyChange = async (topicName, newDifficulty, e) => {
    ...
    // Confirm with user since this will delete old content
  ...
  useEffect(() => {
    const fetchUser = async () => {
      ...
      setIsAuthenticated(true);
        ...
        // You'll need to create this endpoint: GET /daily-progress or similar
        try {
  ...
  useEffect(() => {
    // Construct lessonIdToTopicId from userTopic
    if (userTopic?.topics) {
      const map = {};
  ...
  // Generate weekly activity data from DailyProgress model
  const generateWeeklyData = () => {
    ...
    dailyProgress.forEach(progress => {
      ...
      // Calculate which day of week this is
  ...
  // Course icons and colors mapping
  const courseConfig = {
  ...
  // Add a lessonId to topicId map for daily aggregation
  
  

  // Generate weekly activity data for a specific topic from DailyProgress model
  const generateWeeklyDataForTopic = (topicId) => {
    ...
    dailyProgress.forEach(progress => {
      ...
      if (daysDiff >= 0 && daysDiff < 5) {
        const dayNames = ['Mon', 'Tue', 'Wed', 'Thu', 'Fri'];
        // Count completedTitles for this topic only
                      ...
                      <div className="bg-gray-50 rounded-2xl p-6 w-80">
                        ...
                        <div className="flex items-end justify-between gap-3 h-32">
                          {weeklyData.map((data, index) => {
                            // Use topic's total lessons as the bar's max
