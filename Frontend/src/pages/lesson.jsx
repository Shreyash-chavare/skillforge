...
const Lesson = () => {
  ...
  // Fetch progress for current title
  useEffect(() => {
    const fetchProgress = async () => {
      ...
      try {
        ...
        // Lock the title if score < 80 and there are recommendations
        ...
        // Check if all recommendations already completed
        ...
        if (allCompleted && !currentTitle.completed) {
          ...
          // Optionally call backend (if not already marked)
          await api.post(
            `/progress/mark`,
  ...
  const fetchTitles = async () => {
    ...
    try {
      ...
      // Unlock next title if previous is completed
  ...
  // Mark title as completed when all recommendations are done
  // Mark title as completed when all recommendations are done
useEffect(() => {
    ...
    if (recommend?.length > 0 && currentTitle) {
      ...
      if (allCompleted && !currentTitle.completed) {
        ...
        try {
          ...
          // ✅ Re-fetch updated titles to stay in sync
          await fetchTitles();
        } catch (err) {
    ...
    <div className="p-6 bg-green-300">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-3 gap-6">
          ...
          {/* Right: AI Recommendation + Lottie */}
